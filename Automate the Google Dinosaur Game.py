...
# Step 1: A function for returning the pixel of the trigger point
def get_pixel(image, x, y):
    px = image.load()
    return px[x, y]


# Step 2: Saving the screenshot of the picture to know when there is a change of pixels at the trigger point.
def play_game():
    ...
    # allowing 3s to switch the interface to Google Chrome after the program is executed
    time.sleep(3)
    while True:
        t1 = time.time()
        ...
        for i in reversed(range(x_start, x_end)):
            # the color of the pixel does not match the color of the background color
            ...
            if get_pixel(sct_img, i, y_search_for_bird) != bg_color:
                ...
                keyboard.press("down")
                time.sleep(0.4)
                # release the still pressed down key
        ...
        # game is accelerating if the intervals are not the same
...
•	Press the space button to start game and q to exit/quit the game 
"""
